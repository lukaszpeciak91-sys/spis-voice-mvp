...
object VoskAudioConverter {
    private const val TARGET_SAMPLE_RATE = 16000
    private const val LOG_TAG = "VoskAudioConverter"

    fun convertToPcm16(inputFile: File): ShortArray {
        ...
        return try {
            ...
            Log.i(
                LOG_TAG,
                "Output format sampleRate=${TARGET_SAMPLE_RATE}Hz channels=1 samples=${resampled.size} conversionMs=${elapsedMs}"
            )
            resampled
        } catch (e: Exception) {
    ...
    private fun resample(samples: ShortArray, inputRate: Int, targetRate: Int): ShortArray {
        ...
        for (i in output.indices) {
            ...
            val clamped =
                interpolated.roundToInt().coerceIn(Short.MIN_VALUE.toInt(), Short.MAX_VALUE.toInt())
